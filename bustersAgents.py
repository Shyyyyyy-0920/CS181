    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        #每个未被抓住的幽灵，用当前信念分布中概率最高的位置作为估计位置。
        likelyGhostPositions = [
            distribution.argMax()
            for distribution in livingGhostPositionDistributions
        ]
        if len(likelyGhostPositions) == 0:
            return Directions.STOP

        bestAction = Directions.STOP
        bestDistance = float('inf')

        #枚举 Pacman 的合法动作，选择执行后到“最近目标幽灵”的迷宫距离最小的动作。
        #这里使用 maze distance（会考虑墙），不能用普通曼哈顿距离。
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            closestGhostDistance = min(
                self.distancer.getDistance(successorPosition, ghostPosition)
